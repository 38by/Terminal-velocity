...
function main()
    ...
    for idx in s_dp:in_dp:e_dp
        results, distance, _, _ = simulate(idx, initialV, initialT, initialRe, timestep, ZEP, max_time)
        
        if !isempty(results) && !isnan(distance)  
            push!(fall_distances, distance * 100)  # mmに変換
            push!(par_size, idx)
        else
            println("Skipping dp=$idx um due to invalid distance.")
        end
    end

    output_data = hcat(par_size, fall_distances)
    writedlm("particle_falldistance-noflowpow-2.dat", output_data, ',')

    default(fontfamily = "serif-roman")
    plot(par_size, fall_distances,
        st=:line, 
        lw=2,
        color=:black,
        xlabel="Particle [μm]", 
        ylabel="Fall Distance [mm]", 
        guidefontsize=16,   
        tickfontsize=12,    
        legend=false,
        framestyle=:box,
    ...
    savefig("falldistance-noflowpow-pixel-2.png")
end
